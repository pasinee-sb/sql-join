...
(6 rows)
*/
SELECT first_name, last_name, COUNT(vehicles.id) FROM owners FULL JOIN vehicles  on owners.id = owner_id WHERE vehicles.id IS NOT NULL GROUP BY owners.id ORDER BY first_name;
...
Jane       | Smith     |         15000 |     2
*/
SELECT first_name,last_name, ROUND(AVG(price)) as average_price, COUNT(vehicles.id) FROM owners JOIN vehicles on owners.id = owner_id GROUP BY owners.id HAVING COUNT(vehicles.id)>1 AND ROUND(AVG(price))>10000 ORDER BY first_name DESC;